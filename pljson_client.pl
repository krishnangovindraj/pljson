:- module(pljson_client, [hit_api/3]).
...
% TODO: Handle errors.
% Example usage:  Action = test_protocol(helloworld, X), hit_api("http://127.0.0.1:8008", Action, RL).
...
%  pljson_query(test_protocol(helloworld, X)).
pljson_query(Q):-
    nonvar(Q),
    user:pljson_server_url(Url),
    hit_api(Url, Q, ResultList),
    member(Q, ResultList).
